...
function ScoreChip({ perfect }) {
  const icon = perfect ? (
    ...
    <Target className="h-6 w-6 text-amber-700 dark:text-amber-400" aria-hidden="true" />
  )
...
function QuizResult({ score, total, retry, originalScore, originalTotal, onRetry }) {
  ...
  const cardClass =
    'relative overflow-hidden rounded-2xl border border-stone-200/70 bg-paper/70 px-6 py-8 text-center shadow-paper dark:border-stone-700/60 dark:bg-paper-dark/70'
  const accent = (
    <span
      aria-hidden="true"
  ...
  if (retry) {
    return (
      ...
      >
        ...
        <h3 className="text-lg font-bold text-stone-900 dark:text-white">Retry Complete!</h3>
        <p className="mt-2 text-sm font-medium text-stone-500 dark:text-stone-400">You scored</p>
        <p className="mt-1 text-4xl font-extrabold text-stone-900 dark:text-white">
          {score} <span className="text-xl font-medium text-stone-400 dark:text-stone-500">/ {total}</span>
        </p>
        <div className="mt-2 flex gap-6 text-sm text-stone-600 dark:text-stone-300">
          <p>
        ...
        {typeof originalScore === 'number' && (
          <p className="mt-3 inline-block rounded-full border border-stone-200/80 bg-white/60 px-2.5 py-1 text-xs font-medium text-stone-500 dark:border-stone-700 dark:bg-paper-soft-dark/70 dark:text-stone-400">
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-3 text-lg font-bold text-stone-900 dark:text-white">{headline}</h3>
      <p className="mt-2 text-sm font-medium text-stone-500 dark:text-stone-400">You scored</p>
      <p className="mt-1 text-4xl font-extrabold text-stone-900 dark:text-white">
        {score} <span className="text-xl font-medium text-stone-400 dark:text-stone-500">/ {total}</span>
      </p>
